...
float eulerVel = 0.0f;
float eulerPos = 1000.0f;
float verletPos = 1000.0f;
float verletVel = 0.0f;
/*
...
float euler(float duration){
	eulerPos = eulerPos + duration*eulerVel;
	eulerVel = eulerVel +duration*(-9.8f);

	return eulerPos;
}

float verlet(float duration){
	verletPos = verletPos + verletVel*duration + 0.5f*(-9.8f)*duration*duration;
	verletVel = verletVel + 0.5f*((-9.8f) + (-9.8f))*duration; 
	return verletPos;
}
/*
...
int main() {
  ...
  float position2 = 1000.0f;
  float position3 = 1000.0f;

  float time = 0.0f;
  while (time < 1.0f) {
    // update the position using rk4.
    position = rk4(f, time, position, (1.0f / frequency));
    position2 = euler(1.0f/frequency);
    position3 = verlet(1.0f/frequency);
  ...
  // output.
  std::cout << "The final position is rk4: " << position << std::endl;
std::cout << "The final position is euler : " << position2 << std::endl;
std::cout << "The final position is verlet: " << position3 << std::endl;
  std::cout << "The expected position is: " << expectedY << std::endl;
  std::cout << "The difference for rk4: " << position - expectedY << std::endl;
  std::cout << "The difference for euler: " << position2 - expectedY << std::endl;
  std::cout << "The difference for verlet: " << position3 - expectedY << std::endl;
